...
for(i in 1:length(pheno)){
	df_mantra<-fread(paste0(dirout,'/run_mantra_',pheno[i],'_',type[j],'/mantra.out'),data.table=F,header=F)
	colnames(df_mantra)<-c('rs','chr','ps','all_effect','all_other','nb_study','log10BF','post_proba_hetero','n','direction') 
	ll_no_corr<-read.table(paste0(dirout,'/result_mash_',pheno[i],'_',type[j],'_no_corr.txt'),header=T,nrow=1)
	ll_no_corr<-ll_no_corr[,4]
	ll_simple_corr<-read.table(paste0(dirout,'/result_mash_',pheno[i],'_',type[j],'_simple_corr.txt'),header=T,nrow=1)
	ll_simple_corr<-ll_simple_corr[,4]
	ll_em_corr<-read.table(paste0(dirout,'/result_mash_',pheno[i],'_',type[j],'_em_corr.txt'),header=T,nrow=1)
	ll_em_corr<-ll_em_corr[,4]
	ll<-data.frame(ll_no_corr,ll_simple_corr,ll_em_corr)
	cor_choice<-gsub('ll_','',(names(which.max(ll))))
	print(cor_choice)
	df_mash<-fread(paste0(dirout,'/result_mash_',pheno[i],'_',type[j],'_',cor_choice,'.txt'),data.table=F)
...
venn_plot1<-venn.diagram(X[names(X)%in%c("pc1_varroa_inf_egs_MASH","mnr_egs_MASH","recap_inf_egs_MASH")],filename=NULL,category.names=c("PC1","MNR","Recap"),fill=c("yellow","red","orange"))
...
png(paste0(dirout,'/venn.png'),width=3000,height=1500,res=400)
ggarrange(ggarrange(gTree(children=venn_plot1),gTree(children=venn_plot2),ncol=2),ggarrange(gTree(children=venn_plot3),gTree(children=venn_plot4),gTree(children=venn_plot5),ncol=3),nrow=2)
dev.off()
flog.threshold(ERROR)
venn_plot1<-venn.diagram(X[names(X)%in%c("pc1_varroa_inf_egs_MASH","pc1_varroa_inf_freq_MANTRA","pc1_varroa_inf_egs_MANTRA")],filename=NULL,category.names=c("egs_MAHS","freq_MANTRA","egs_MANTRA"),fill=c("orange","green","dark green"))
...
png(paste0(dirout,'/venn_egs_freq.png'),width=5000,height=1000,res=400)
...
write.table(X,paste0(dirout,'/sign_locus.txt'),col.names=T,row.names=F,quote=F,sep=';')

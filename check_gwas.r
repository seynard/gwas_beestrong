...
pheno_list<-pheno_list[complete.cases(pheno_list),]
...
gwas%<>%mutate(significant=svalue<thresh)
...
g3<-ggplot(df,aes(x=betahat,y=PosteriorMean,colour=1/se))+
        ...
        theme_bw()
g4<-ggplot(df,aes(x=af,y=beta,colour=1/se))+
    geom_point(alpha=0.5)+
    ##geom_smooth()+
    scale_colour_viridis_c()+ 
    geom_vline(xintercept=c(0.1,0.9))
df<-gwas%>%filter(svalue<thresh)
...
qseuil<-max(gwas$p_wald[gwas$qvalue<thresh],na.rm=T)
sseuil<-max(gwas$p_wald[gwas$svalue<thresh],na.rm=T)
...
grid.arrange(g61,g62,g63,nrow=3)
dev.off()
LAMBDA<-data.frame(gwas)
pv=as.data.frame(gwas%>%select(p_wald))
pv<-subset(pv,!is.na(pv[,1]))
dat = as_tibble(pv[,1])
nmax=nrow(dat)
...
y = c(dat$loval,rev(dat$hival))
poly.dat=tibble(x,y)
chisq <- qchisq(1 - pv[,1], 1)
lambda <- median(chisq,na.rm=T) / qchisq(0.5, 1)
LAMBDA$lambda<-lambda
LAMBDA$pheno<-phenotype_id

log_file=paste0(dir_out,'/gemma_',population,'_',phenotype_id,'_',grm_id,'_lmm_',gwas_id,'_cov.log.txt')
log=readLines(log_file)
n<-grep('pve',log)
pve<-as.numeric(tail(unlist(strsplit(log[n[1]],' ')),1))
se_pve<-as.numeric(tail(unlist(strsplit(log[n[2]],' ')),1))
LAMBDA$pve<-pve
LAMBDA$se_pve<-se_pve
LAMBDA$grm<-grm_id
LAMBDA$gwas<-gwas_id
write.table(LAMBDA,paste0(dir_out,'/summary_gwas_',prfx,'.txt'),col.names=T,row.names=F,quote=F)
}
